...
class Lock {
  ...
  private:
    ...
    List *threadQueue; // Queue that will hold threads waiting to acquire lock
    bool lockIsFree; // Denotes wether or not the lock is free
    Thread *threadHoldingLock; // Placeholder variable that denotes the current thread holding the lock
};
...
class Condition {
  ...
  private:
    char* name;
    // plus some other stuff you'll need to define
    // List that holds the sleeping threads for the conditional code
    List *blockedThreads;
};
